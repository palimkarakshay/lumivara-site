...
| 6 | **AI cost numbers don't reconcile** — `cost-analysis 03` Part D shows $280 USD/mo at 25 clients, but the only listed plan above $200 is "hire a second seat." | 🟠 High | `storefront/03` | `18 §3` (was `13 §2`) — closed |
...
| 9 | **Tier 0 economics are loss-leading and undocumented as such** — $90/change with no tracking system, no autopilot, manual operator time. Honest as portfolio play; broken as a tier. | 🟡 Medium | `storefront/02 Tier 0`, `07 Dummy F` | `14 §1` |
...
3. **Run C (1 h Sonnet):** rationalise the maths in `04`, `09`, and `storefront/03`. Reconcile, update tables. Use prompt in `16 §3`.
...
- Marketing / sales process — `storefront/01` is fine; not the bottleneck.
