...
The single most important verdict: **the PoC mostly does not need further perfection — but the demo-readiness gate (§8) does, and it has named gaps the original review missed.** Section §1 below answers the operator's direct question; §2 maps each review-section to a fix; §3 is the Sales Sprint S0 design; §5 is the resequenced project plan with Phase 1A\* demo-readiness inserted; §8 is the demo-readiness gate (G1–G4 + 8.2 checklist); §9 is the delivery-risk fallback stack; §10 is the Dual-Lane separation status; §11 is the gap inventory (G5–G27 + H1–H8) with a 5.5/10 codebase-audit score; §12 is the prioritised operator-vs-automation task list; §13 is the tech-stack and automation-approach alternatives.
...
The PoC is good enough on the operator-side platform polish dimension (the §1.1 streak gate in `01-poc-perfection-plan.md`). The platform built so far — n8n workflows exported, multi-vendor LLM fallback, HMAC handshake (production-grade per `src/lib/admin/webhooks.ts`), `assertSafePromotion` deploy guard, plan-then-execute pipeline, axe-core + Lighthouse CI gates, the four-tier cron cadence, `llm-monitor` self-awareness — is **at-or-above** the level a paying solo dentist would notice or pay extra for. The §1.1 streak gate ("10 consecutive auto-routine issues land green with zero operator intervention") is an **internal quality bar**, not a customer-impact bar. A prospect does not buy "10 consecutive green issues." A prospect buys *"my site got faster, my recall flow is working, and I texted you a change yesterday and it shipped this morning."*

**However** — and this is the nuance the original review missed — the **demo-readiness** dimension is genuinely incomplete. The independent codebase audit returned a delivery-readiness score of **5.5/10** (see [§11.5](#115--revised-delivery-readiness-score)) and named ~15 gaps the original review didn't surface. The most consequential of those: G5 (Vercel env vars not set), G6 (n8n instance not provisioned + workflows not imported), G9 (Resend DNS authentication missing — directly affects Sales Sprint S0 reply rates), and the four operator-named gaps G1–G4. **These are demo-blockers**, time-boxed at 2–4 weeks per Phase 1A\* in [§5.1](#51--the-new-phase-map). They are real, but they are *configuration and missing-middle work*, not "rebuild the platform" work — see [§11](#11--additional-gaps-g5g24-surfaced-2026-05-01) for the inventory and [§13](#13--tech-stack-and-automation-approach-alternatives) for the swaps that shrink the surface to fix.
...
|---|---|---|---|---|---|
| M1 | Pre-revenue, post-PMF posture (`§1.1`) | Critical | Run **Sales Sprint S0** (§3 below) — 50 prospects, one vertical, 90-day time-box. **Prerequisite: Phase 1A\* demo-readiness gate (§8 + G1–G9 in §11.1) closes in 2–4 weeks, OR the §9.1 honest-pitch stack is committed in writing.** | Operator | Reply rate measured + at least 1 booked discovery call OR sprint declared failed and re-run with different vertical. |
...
| M15 | Platform built before client #2 (`§3`) | Critical | **Hard freeze on platform work, with one carve-out.** No new platform features land between today and client #2 closing — **except** the Phase 1A\* demo-readiness fixes (G1 n8n credentials, G2 inline preview embed, G3 publish-button auto-deploy, G4 Path-B ownership-claim language, G5 Vercel env vars, G6 n8n provisioning, G9 Resend DNS). Those are demo-blockers, not features. Pipeline-lane PRs that are platform-improvement-only and not on the G-list get the `frozen-until-client-2` label and are not auto-merged. Bug fixes and security patches are exempt. | Operator | CI rule + label exists; PR titles audited weekly; G-list PRs labelled `phase-1a-demo-readiness`. |
...
> _§5.6 is the **operator-side** week-1 plan. The matching **automation-side** week-1 plan lives in [§12.2 P0 rows](#122--automation-blocking-tasks-bot-runnable-label-auto-routine), which run in parallel via the auto-routine bot pipeline. Together they are the Phase 1A\* demo-readiness gate (§8) closing in week 1; the rest of Phase 1A\* takes weeks 2–4 (G6 n8n provisioning + G7 Stripe deferred per §13.1 S4 + the rest of §12)._
