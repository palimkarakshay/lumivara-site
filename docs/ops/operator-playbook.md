...
| **`paper-only`** | Brand-drift sweep: find every issue or doc reading "Lumivara Infotech" and either close-as-`status/post-migration` or rename to `Lumivara Forge`. (Today's known offenders: issues #107, #110, #111, #112.) | `gh issue list --search "Infotech in:title,body"` | Cleans §6.5 brand-lock audit pre-emptively. |
