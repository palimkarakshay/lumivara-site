...
| **`paper-only`** | Brand-drift sweep on **GitHub Issues** only: find every issue reading the retired working name (search the keyword `Infotech`) and either close-as-`status/post-migration` or rename to `Lumivara Forge`. (Known offenders: issues #107, #110, #111, #112.) The doc-side sweep landed in PR #200 (2026-04-29); `scripts/pattern-c-audit.sh` keeps it green going forward. | `gh issue list --search "Infotech in:title,body"` | Cleans §6.5 brand-lock audit pre-emptively. |
...
| **The catalog you're selling** | [`docs/decks/04-prospective-client-deck.md`](../decks/04-prospective-client-deck.md), [`docs/freelance/01-gig-profile.md`](../freelance/01-gig-profile.md) |
