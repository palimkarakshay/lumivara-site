...
| **GitHub App** (`{{BRAND_SLUG}}-pipeline-bot`) | Org-level App, installed per-engagement on each `<slug>-site` repo | The cross-repo bridge: Issues / Pull requests / Contents / Metadata / Workflows scoped to one site repo per installation; mints ≤ 1 h installation tokens at workflow run time | The matched site repo only; cannot read other clients' repos or the operator vault. Canonical model in [`03 §3.X`](03-secure-architecture.md#3x-github-app-identity-model-canonical-vendor-auth). Replaces the deprecated `VENDOR_GITHUB_PAT` vendor identity (see `12 §4`). |
