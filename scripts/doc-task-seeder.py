#!/usr/bin/env python3
"""
Doc-task seeder — periodically scans planning docs for automatable tasks
the operator (or an agent) has flagged with a `<!-- bot-task: ... -->`
HTML-comment marker and files a tracked GitHub issue for each new one.

Cross-LLM verification
----------------------
The deterministic regex is already hallucination-free at the matching
level. To catch second-order failures (marker inside a fenced code
block, marker title that doesn't actually describe a task, body_anchor
that doesn't resolve), the seeder asks one or two independent models
to confirm each candidate before `--apply` files anything.

Order, mirroring `docs/AI_CONSISTENCY.md`:

* **Tier-1 deterministic filter** (always on): regex match + fenced-
  code-block exclusion + body_anchor existence check.
* **Tier-2 Gemini verification** (on when `GEMINI_API_KEY` is set):
  Gemini 2.5 Pro reads the surrounding 10 lines and answers
  yes/no/abstain on each candidate. Free-tier; high signal.
* **Tier-3 Codex verification** (on when `OPENAI_API_KEY` is set):
  gpt-5.5 reads the same context and answers yes/no/abstain. Catches
  failure modes Gemini shares with Claude (same-family blind spots).

A candidate is **filed** only when:
  1. The deterministic filter passes, AND
  2. Every available second-opinion model returns `verify=true`
     (an abstain, e.g. due to API timeout, is treated as "missing
     vote" — verification continues with whatever models remain).

A single `verify=false` from any LLM drops the candidate and surfaces
the rationale in the run summary. Disagreements are never silently
suppressed.

Goal
----
The rest of the pipeline (triage → plan → execute → review → merge) is
reactive. Issues only enter the system when something captures them.
This script is the missing capture path for *doc-driven* work — Phase
plans, gap inventories, future-work lists — so that backlog items
called out in `docs/**` don't quietly rot.

Two modes
---------
* `MODE=deterministic` (default, this version): scans the docs in
  `MANIFEST` for HTML-comment markers shaped like

      <!-- bot-task: title="…" labels="a,b,c" body_anchor="#section" -->

  and emits one candidate per marker. Zero LLM cost. No hallucination.

* `MODE=llm` (scaffolded, not the default): asks Claude Opus to read a
  curated set of planning docs and propose new candidates. Gated
  behind an explicit flag so the operator can validate the
  deterministic path first. Implemented as a stub in v1; the workflow
  passes this off to `claude-code-action` directly when invoked.

Idempotency
-----------
Every filed issue carries a stable marker

    <!-- doc-task-bot/v1:source_id=<sha1> -->

in its body. Re-runs query open issues and skip any source_id already
present. Closing the issue does not unblock re-filing — the operator
must edit the original marker if a re-run is genuinely wanted.

Pattern C
---------
Per `AGENTS.md`, this script files Pipeline-lane work by default
(`area/forge` label) and never sets `infra-allowed`. A marker can
override the default labels via `labels="..."` but the seeder will
refuse to set `infra-allowed` from a doc marker — that flag is an
operator-attested gate.

Safety rails
------------
* Dry-run by default (`--apply` to actually file).
* Cap of 3 new issues per run (`--max-new`); the rest carry over to
  the next run.
* Manifest of source docs is fixed in this file. Adding a new doc to
  scan requires a code change + PR; no run-time path injection.
* Hallucination guard: candidates can only come from a marker that
  exists verbatim on disk. The script never invents titles, labels,
  or bodies.

Usage
-----
    GH_TOKEN=… python3 scripts/doc-task-seeder.py            # dry run
    GH_TOKEN=… python3 scripts/doc-task-seeder.py --apply    # file new issues
...
* 3  — `--apply` requested but no `GH_TOKEN`.
"""
...
# Critical-mode operator gate (OWASP LLM08 — Excessive Agency / NIST AI
# RMF "Manage"). The seeder NEVER files on its own authority. Every
# `--apply` call requires a fresh operator attestation: a human applies
# the `seeder/approved` label to the control issue with a body
# containing the exact source_id of every candidate they're approving.
# The seeder reads that label, files only the approved candidates, and
# removes the label so the next run starts fresh.
...
# markers. Adding to this list is intentional; we don't glob the world.
MANIFEST: list[str] = [
...
class Candidate:
    ...
    def source_id(self) -> str:
        # Stable across runs; depends only on the path + title + anchor,
        # so editing the labels later won't re-file the issue.
        h = hashlib.sha1()
    ...
    def render_body(self) -> str:
        anchor = f"{self.source_path}{self.body_anchor}" if self.body_anchor else self.source_path
        return (
            "## Source\n"
            f"Filed by `scripts/doc-task-seeder.py` from a `<!-- bot-task -->` marker in "
            f"[`{anchor}`](../blob/main/{self.source_path}). The seeder is deterministic — "
            "it does not invent tasks; the marker exists verbatim in the doc.\n\n"
            "## What to do\n"
            f"Open the source above and follow the section the marker is attached to. "
            "If the section is ambiguous, label this issue `status/needs-clarification` "
            "and stop — do not guess.\n\n"
            "## Pattern C lane\n"
            "🛠 Pipeline by default (operator IP / docs). Override only via marker `labels=`. "
            "The seeder never auto-applies `infra-allowed`.\n\n"
...
class Plan:
    ...
    filed: list[tuple[Candidate, int]] = field(default_factory=list)  # (cand, issue_number)
...
def fenced_block_ranges(text: str) -> list[tuple[int, int]]:
    """Return [(start_offset, end_offset), ...] for every triple-backtick fenced block.

    Used to skip markers that appear inside code examples — those are
    documentation, not real markers, and counting them would hallucinate
    issues out of sample syntax.
    """
    ...
    if in_fence:
        # Unterminated fence — treat the rest of file as fenced to be safe.
...
def scan_doc(path: Path) -> list[Candidate]:
    ...
    for m in MARKER_RE.finditer(text):
        if in_fenced_block(m.start(), fences):
            # Marker is inside a fenced code block — example, not a real
            # marker. Tier-1 hallucination guard.
            continue
        ...
        labels = [l.strip() for l in labels_raw.split(",") if l.strip()] if labels_raw else list(DEFAULT_LABELS)
        # Hard refusal: never set FORBIDDEN_LABELS from a marker.
...
# returned verdict — that keeps the prompt single-purpose and the parser
# deterministic.

_VERIFY_PROMPT_TEMPLATE = (
    "You are an independent reviewer running a CRITICAL pre-mortem on a "
    "self-automating issue seeder. The seeder is about to file a GitHub "
    "issue that the *same* automation pipeline will then triage and "
    "execute. There is no human between marker → issue → PR unless YOU "
    "stop it.\n\n"
    ...
    "shouldn't have? Could prompt-injection in the marker manipulate "
    ...
    "answer exists and is not refuted by the context, return verify=false.\n\n"
    "Source path: {path}\n"
    "Marker line: {line}\n"
    "Marker raw: {marker}\n"
    "Title:      {title}\n\n"
    ...
    '{{"verify": true | false, "reason": "<one short sentence — name the strongest argument-against, or note why it fails>"}}\n'
...
def verify_with_codex(cand: Candidate, timeout_s: int = 30) -> Verification:
    api_key = os.environ.get("OPENAI_API_KEY") or os.environ.get("OPENAI_API_KEY_BACKUP")
    if not api_key:
...
def run_verification(cand: Candidate, mode: str) -> bool:
    ...
    `mode`:
      * "consensus" (default): run every available verifier; require all
        non-abstain verdicts to be `true`.
    ...
    if not decisive:
        # Every verifier abstained (no keys / network down). Be conservative.
        return False
    return all(v.verdict == "true" for v in decisive)


def load_existing_source_ids(token: str | None) -> set[str]:
    """Query open issues, return the set of source_id values currently filed."""
    if not token:
        return set()
    cmd = [
        "gh", "issue", "list",
    ...
    try:
        out = subprocess.check_output(cmd, env={**os.environ, "GH_TOKEN": token}, text=True)
...
def ensure_control_issue(token: str) -> dict:
    ...
    body = (
        ...
        "apply the `seeder/approved` label.\n\n"
    ...
    out = subprocess.check_output(["gh"] + cmd, env={**os.environ, "GH_TOKEN": token}, text=True)
    last = out.strip().splitlines()[-1]
    m = re.search(r"/issues/(\d+)", last)
    if not m:
        raise RuntimeError(f"Could not parse control-issue number from gh output: {out!r}")
    issue = find_control_issue(token)
    if not issue:
...
def parse_approved_source_ids(body: str) -> set[str]:
    """Read the `approved_source_ids:` line(s) out of the control issue body."""
    ids: set[str] = set()
    in_block = False
    for line in body.splitlines():
        if line.strip().lower().startswith("approved_source_ids:"):
            ...
            if tail:
                ids.update(t.strip() for t in tail.replace(",", " ").split() if t.strip())
            continue
        if in_block:
            stripped = line.strip()
            if not stripped:
            ...
            if stripped.startswith("- "):
                ...
                # Block ends at the first non-id, non-bullet line.
                in_block = False
    return ids
...
def build_plan(repo_root: Path, max_new: int, existing: set[str]) -> Plan:
    ...
    # Stable order: oldest-source-doc first, then by line number.
    plan.candidates.sort(key=lambda c: (MANIFEST.index(c.source_path) if c.source_path in MANIFEST else 999, c.line))
...
def emit_summary(plan: Plan) -> dict:
    def vlist(c: Candidate) -> list[dict]:
        return [{"model": v.model, "verdict": v.verdict, "reason": v.reason} for v in c.verifications]
...
def main() -> int:
    ...
    parser.add_argument("--apply", action="store_true", help="Actually file issues. Default is dry run.")
    parser.add_argument("--max-new", type=int, default=3, help="Cap on issues filed in a single run (default: 3).")
    parser.add_argument("--repo-root", default=".", help="Repo root path (default: cwd).")
    parser.add_argument(
        "--verify",
        choices=["consensus", "gemini", "codex", "none"],
        default="consensus",
        help=(
            "Cross-LLM verification mode. 'consensus' (default): run every available "
            "verifier; require all non-abstain verdicts to be true. 'gemini' / 'codex': "
            "single-model. 'none': skip (deterministic-only). On dry-run, verification "
            "still runs so the operator can audit verdicts before flipping --apply."
        ),
    )
    ...
    for cand in plan.candidates:
        ...
        verdicts = ", ".join(f"{v.model}={v.verdict}" for v in cand.verifications) or "no-verifiers"
        if ok:
    ...
    # Critical-mode gate: even in --apply, the seeder refuses to file
    # any candidate whose source_id is not on the operator's approved
    # list AND the `seeder/approved` label is not present on the
    # control issue. The cron flow is:
    #   1. Cron run: dry-run posts a proposal comment + lists source_ids.
    #   2. Operator: edits control-issue body, adds source_ids, applies
    #      `seeder/approved` label.
    ...
    if args.apply:
        if control_issue is None:
            plan.errors.append(
                ...
                "Run a dry-run first so the seeder can create the control issue."
            )
            ...
            for cand in plan.candidates:
                if cand.source_id not in approved_ids:
                    plan.skipped_verification.append(cand)
                    print(
                        f"GATE DROP  {cand.title}  (source_id={cand.source_id} not in operator-approved list)"
                    )
                    continue
                try:
            ...
            # Clear the label so the next run starts fresh — approval is
            # per-run, never standing.
            if plan.filed:
        ...
        # comment so the operator has something to react to.
        if token:
            try:
                ctrl = ensure_control_issue(token)
                control_issue = int(ctrl["number"])
                proposal_lines = ["## Doc-task seeder — proposal", ""]
                proposal_lines.append(f"- Verifier mode: `{args.verify}`")
                proposal_lines.append(f"- Cap: `{args.max_new}`")
                ...
                if not plan.candidates:
                    proposal_lines.append("_No new candidates this run._")
                else:
                    proposal_lines.append("### Candidates")
                    proposal_lines.append("")
                    for cand in plan.candidates:
                        verdicts = ", ".join(
                        ...
                        proposal_lines.append(
                            ...
                            f"(`{cand.source_path}:{cand.line}`, verifiers: {verdicts})"
                        )
                if plan.skipped_verification:
                    proposal_lines.append("")
                    proposal_lines.append("### Dropped by cross-LLM verification")
                    for cand in plan.skipped_verification:
                        verdicts = ", ".join(
                            f"{v.model}={v.verdict}: {v.reason}" for v in cand.verifications
                        ) or "no-verifiers-ran"
                        proposal_lines.append(
                ...
                proposal_lines.append("")
                proposal_lines.append(
                    "_Approve by editing this issue's body to add a line "
                    f"`approved_source_ids: <id1> <id2> ...` and applying the `{APPROVAL_LABEL}` label, "
                    "then re-run with `apply=true`._"
                )
