...
| [`doc-task-seeder.md`](doc-task-seeder.md) | Operator contract for `doc-task-seeder.yml` (weekly Sun 02:00 UTC cron) and the deterministic `<!-- bot-task: ... -->` marker the seeder consumes. The capture layer for doc-driven backlog — without it, `[ ]` rows in planning docs rot until manually surfaced. Dry-run by default; `--apply` is operator-attested via `workflow_dispatch`. |
