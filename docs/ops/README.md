...
| [`automation-map.md`](automation-map.md) | Single-page index of every workflow, its cron, concurrency group, and the script it calls. The first place to look when wondering "what runs and when?" Includes lane-exclusivity rules, maker-checker pairs, and known limitations. |
| [`automation-future-work.md`](automation-future-work.md) | Gap analysis + concrete next-PR shopping list for new workflows / scripts / crons. Leads with the missing PR-gating CI workflow as the highest-leverage gap; covers 24/7 utilisation knobs aligned with the Max 20x posture; cites OWASP / SRE / GitHub Actions hardening standards per item. |
