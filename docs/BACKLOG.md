...
- **Doc-driven capture**: `.github/workflows/doc-task-seeder.yml` runs every Sunday and scans curated planning docs for `<!-- bot-task: ... -->` markers, filing one issue per new marker (dry-run by default; operator flips `--apply` via `workflow_dispatch` once the dry-run candidate list looks right). Contract + manifest in [`docs/ops/doc-task-seeder.md`](./ops/doc-task-seeder.md).
