...
| 10 | `10-pivot-execution-playbooks.md` | **Tactical playbook layer beneath `09 §3`.** Common pre-flight (§1), per-pivot playbooks A–K (§§2–12) with prerequisites + week-by-week steps + success metrics + failure detection + costs + 4-row impact summaries, cross-pivot impact matrix (§13), common failure modes across pivots (§14), and verified May 2026 pricing + URLs + 7-day kickoff (§17 — canonical reference; supersedes earlier sections where conflicting). | Operator on a Tuesday after picking which pivot(s) to execute |
| 11 | `11-archive-execution-strategy.md` | **Tactical archive layer beneath `09 §5`.** Pre-archive flight check (§1), three mechanism patterns P1/P2/P3 (§2), per-bucket plans (§§3–10) with order-of-operations + commands + verification gates + impact, contamination resolution (§11), cross-bucket impact summary (§12), dual-lane teardown (§13), 5-command verification gate (§14), rollback paths (§15), six common pitfalls (§16), 18-step day-of master sequence (§18). | Operator on archive day, before the first `git mv` runs |
