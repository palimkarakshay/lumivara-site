#!/usr/bin/env python3
"""
Generate the rolling backlog digest — the operator's always-fresh brief.
...
This is a *narrative* counterpart to `bot-usage-report.py`:
  ...
  * `backlog-digest.py` answers "what shipped, what's stuck, what's
    queued?" — issue/PR-motion summary, runs every 2h round the clock.
...
Inputs (env):
    ...
    LOOKBACK_DAYS     default: 7   (rolling shipping window)
...
def render(now: datetime,
    ...
    # Dual-Lane Repo state — single tracking issue from dual-lane-watcher.yml.
    dual_lane_state = "_no tracking issue found (watcher last run was green)_"
    for i in open_issues:
        if i["title"] == "Dual-Lane watcher: drift detected":
            dual_lane_state = (
    ...
    parts: list[str] = [
        ...
        "This issue is rewritten every 2h — refresh the page for the "
        "current state. Older snapshots are in the issue's edit history._",
        "",
        "## At a glance",
        "",
        ...
        f"- **Dual-Lane watcher:** {dual_lane_state}",
        "",
    ]
    ...
    parts += [
        ...
        "`.github/workflows/backlog-digest.yml` (every 2h, offset :37). "
        "Cross-references: `bot-usage-monitor.yml` (provider dashboard), "
        "`dual-lane-watcher.yml` (charter drift), "
